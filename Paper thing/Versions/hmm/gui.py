...
class GUI(tk.Tk):
    ...
    def rotate(self, event):
        print("Rotating")
        self.rotateRelPos = [event.x - self.rotateStartPos[0], self.rotateStartPos[1] - event.y]
        # print(self.rotateRelPos)
        self.angle = math.sqrt(self.rotateRelPos[0]**2 + self.rotateRelPos[1]**2)
        self.angle = math.radians(self.angle)
        self.axis = [(self.rotateRelPos[i] / self.angle) for i in range(2)]
        self.axis.append(0)
        print(self.angle)
        print(self.axis)
        self.update()

...
class GUI(tk.Tk):
    ...
    def __init__(self):
        ...
        self.bottomPanelButtons.append([ttk.Label(self.bottomPanel, text="Fill:", background=self.backgroundColor), tk.Checkbutton(self.bottomPanel, background=self.backgroundColor, command=self.update_canvas, variable=self.fill)])
        self.fill.set(True)
        ...
        for i in range(len(self.bottomPanelButtons)):
            ...
            if self.bottomPanelButtons[i] == 0:
    ...
    def update_canvas(self):
        ...
        for i in range(len(self.object.faces)):
            ...
            if len(vertexNormals) > 0 and self.fill.get():

...
for p in relevant:
	...
	cnt = 0
	for p2 in relevant:
		temp = (1-(math.sqrt((p["x"]-p2["x"])**2+(p["y"]-p2["y"])**2+(p["z"]-p2["z"])**2)/(0.5*maxPath/r))**2)
		if temp > 0:
			density += temp
			cnt += 1
		#          |---------- p és p2 relatív távolsága (0 és 1 közé eső érték) (ezeket átlagolva lesz sűrűség) ----------|
		#          |- Ha p és p2 ugyanott vannak akkor 1, ha maxPath/(2*r) vagy nagyobb távolságra vannak akkor 0 az érték-|
	density /= cnt
	...
	p["weight"] = density * (p2["e"]/maxE) * (1-p["distance"]/(0.5*maxPath))
...
for p in points:
	if p in relevant:
		...
		if p["density"] == maxDensity:
			...
			canvas.create_oval(p["x"]*size-(maxPath/(2*r))*size+margin, p["y"]*size-(maxPath/(2*r))*size+margin, p["x"]*size+(maxPath/(2*r))*size+margin, p["y"]*size+(maxPath/(2*r))*size+margin, outline="red")
	else:
		modWeight = -1

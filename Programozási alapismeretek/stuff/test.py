...
for p in relevant:
	...
	density = 0
	for p2 in relevant:
		density += (1-(math.sqrt((p["x"]-p2["x"])**2+(p["y"]-p2["y"])**2+(p["z"]-p2["z"])**2)/(maxPath/2))**2)
		#          |--- p és p2 relatív távolsága (0 és 1 közé eső érték) (ezeket átlagolva lesz sűrűség) ---|   |----- a gyöngy értéke -----|
		#          |-- Ha p és p2 ugyanott vannak akkor 1, ha maxPath/2 távolságra vannak akkor 0 az érték --|   |- (0 és 1 közé eső érték) -|
	...
	p["weight"] = density * (p2["e"]/maxE) * (1-p["distance"]/(maxPath/2))
